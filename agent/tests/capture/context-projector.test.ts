...
import {
  projectCapturedAccessibility,
  type AccessibilityProjectionDiagnostics,
} from "../../src/capture/accessibility-projector.js";
import { projectCapturedContext } from "../../src/capture/context-projector.js";
...
function artifact(options?: {
  ...
  if (hasScreenshot) {
    capture.persistence = {
      structuredPath: "/private/capture-1.json",
      screenshot: {
        path: "/private/capture-1.jpg",
        mimeType: "image/jpeg",
        width: 100,
        height: 80,
        sha256: "hash",
...
test("projects a persisted JPEG and bounded AX data from one artifact", async (t) => {
  ...
  assert.equal(context.requestId, "observation-1");
  assert.equal(context.diagnostics.intentRevision, 6);
  assert.equal(context.diagnostics.artifactRevision, 4);
  assert.equal(context.diagnostics.completedRevision, 4);
  assert.equal(context.diagnostics.intentContentEpoch, 5);
  assert.equal(context.diagnostics.artifactContentEpoch, 3);
  assert.equal(context.diagnostics.completedContentEpoch, 3);
  assert.equal(context.image?.mimeType, "image/jpeg");
  assert.equal(context.image?.width, 100);
  assert.equal(context.image?.height, 80);
  assert.equal(context.image?.path, "/private/capture-1.jpg");
...
test("reuses the JPEG already persisted with the capture artifact", async (t) => {
  ...
  assert.equal(context.image?.path, persistedPath);
});
...
test("preserves AX-only partial success without inventing an image", async (t) => {
  ...
  assert.equal(context.status, "ax_only");
...
test("keeps the screenshot when AX projection fails", async (t) => {
  ...
  assert.equal(context.status, "screenshot_only");
...
test("does not project AX when the accessibility modality failed", () => {
  assert.equal(
    projectCapturedAccessibility(artifact({ accessibility: false })),
    undefined,
  );
});

test("reports unavailable when neither persisted image nor useful AX exists", async (t) => {
  const dataRoot = await mkdtemp(join(tmpdir(), "openscreen-screen-context-"));
  t.after(() => rm(dataRoot, { recursive: true, force: true }));

  const context = await projectCapturedContext(
    artifact({ screenshot: false, accessibility: false }),
    "request-1",
  );
  ...
  assert.equal(context.image, undefined);
  assert.equal(context.accessibility, undefined);
  assert.equal(context.diagnostics.screenshotStatus, "unavailable");
  assert.equal(context.diagnostics.accessibilityStatus, "unavailable");
});
...
test("omits AX that only repeats the application and window titles", async (t) => {
  ...
  const diagnostics: AccessibilityProjectionDiagnostics[] = [];
...
test("projects useful AX content and excludes shell subtrees and duplicates", () => {
  ...
  const projection = projectCapturedAccessibility(capture);
