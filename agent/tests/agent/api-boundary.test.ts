...
const agentRoot = resolve("agent/src/agent");
...
test("keeps model enumeration and switching out of the public Agent contract", () => {
  const files = [
    ...
    resolve("agent/src/application/api.ts"),
    resolve("agent/src/application/runtime.ts"),
    resolve("agent/src/transport/jsonl-codec.ts"),
  ];
  ...
  for (const file of files) {
    ...
    assert.doesNotMatch(
      ...
      relative(resolve("agent/src"), file),
    );
  }
});
