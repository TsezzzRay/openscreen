...
export class CapturePlanner {
  ...
  constructor(
    private readonly config: CaptureConfig["scheduling"],
  ) {}
  ...
  push(
    ...
    const delay = this.config.delaysMilliseconds[
      signal.kind as keyof CaptureConfig["scheduling"]["delaysMilliseconds"]
    ];
    if (delay === undefined) {
    ...
    const cap = this.config.capsMilliseconds[
      signal.kind as keyof CaptureConfig["scheduling"]["capsMilliseconds"]
    ];
