...
import type { AccessibilityNode } from "./protocol.js";

const MODEL_AX_MAX_CHARACTERS = 10_000;
const MODEL_AX_MAX_TEXT_BLOCK_CHARACTERS = 2_048;

export type ModelAccessibilityProjectionDiagnostics = {
  included: boolean;
  omittedReason?:
    | "capture_unavailable"
    | "no_useful_content"
...
type ModelAccessibilityProjection = {
  context?: ModelScreenContext;
  diagnostics: ModelAccessibilityProjectionDiagnostics;
};
...
export function projectModelScreenContext(
  artifact: CaptureArtifact,
): ModelScreenContext | undefined {
  return projectModelScreenContextWithDiagnostics(artifact).context;
}

function projectModelScreenContextWithDiagnostics(
  artifact: CaptureArtifact,
): ModelAccessibilityProjection {
  ...
  let projection: ModelScreenContext = {
  ...
  for (const element of normalized.elements) {
    ...
    if (JSON.stringify(next).length > MODEL_AX_MAX_CHARACTERS) {
  ...
  if (JSON.stringify(projection).length > MODEL_AX_MAX_CHARACTERS) {
    throw new Error("Accessibility model projection exceeds its fixed bound");
  }
  return {
    context: projection,
    diagnostics: {
...
export async function materializeTurnScreenContext(
  dataRoot: string,
  artifact: CaptureArtifact,
  ...
  let accessibility: ModelScreenContext | undefined;
  try {
    const projected = projectModelScreenContextWithDiagnostics(artifact);
  ...
  let image: TurnScreenContext["ref"]["image"];
  ...
  if (
    ...
    image = await persistScreenshot(dataRoot, artifact.captureId, {
      data: Buffer.from(screenshot.dataBase64, "base64"),
...
function normalizeUsefulAccessibility(
  ...
  elements: NonNullable<ModelScreenContext["elements"]>;
  ...
  const elements: NonNullable<ModelScreenContext["elements"]> = [];
...
function boundTextBlock(value: string) {
  ...
  if (characters.length <= MODEL_AX_MAX_TEXT_BLOCK_CHARACTERS) return value;
  return characters
    .slice(0, MODEL_AX_MAX_TEXT_BLOCK_CHARACTERS - 1)
    .join("") + "…";
}
...
): Promise<NonNullable<TurnScreenContext["ref"]["image"]>> {
  if (!/^[A-Za-z0-9-]+$/.test(captureId)) {
    throw new Error("Invalid capture identifier");
  }
  const directory = join(dataRoot, "screen-captures");
  await mkdir(directory, { recursive: true, mode: 0o700 });
  await chmod(directory, 0o700);
  const path = join(directory, captureId + ".jpg");
  const temporaryPath = join(directory, "." + captureId + "." + randomUUID() + ".tmp");
  await writeFile(temporaryPath, screenshot.data, { mode: 0o600 });
...
function fitString(
  ...
  maximum: number,
  build: (value: string) => ModelScreenContext,
) {
  ...
  while (low < high) {
    ...
    if (JSON.stringify(build(candidate)).length <= MODEL_AX_MAX_CHARACTERS) {
      low = middle;
    } else {
      high = middle - 1;
    }
