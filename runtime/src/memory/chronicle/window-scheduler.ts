export function chronicleWindowFor(
  occurredAt: number,
  { windowMilliseconds, graceMilliseconds }: {
    windowMilliseconds: number;
    graceMilliseconds: number;
  },
): { id: string; startAt: number; endAt: number; eligibleAt: number } {
  if (
    !Number.isSafeInteger(occurredAt) || !Number.isSafeInteger(windowMilliseconds) ||
    windowMilliseconds <= 0 || !Number.isSafeInteger(graceMilliseconds) ||
    graceMilliseconds < 0
  ) {
