export function turnMemoryBatchEligibility({
  ...
  if (
    !Number.isSafeInteger(projectedInputTokens) ||
    projectedInputTokens < 0 ||
    !Number.isSafeInteger(maxInputTokens) ||
    maxInputTokens <= 0 ||
    !Number.isSafeInteger(idleMilliseconds) ||
    idleMilliseconds <= 0 ||
    !Number.isSafeInteger(hardCapMilliseconds) ||
    hardCapMilliseconds <= 0
  ) {
    throw new Error("Invalid Turn Memory batch policy");
  }
